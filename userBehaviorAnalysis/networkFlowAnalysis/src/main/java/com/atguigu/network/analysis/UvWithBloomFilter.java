...
public class UvWithBloomFilter {
    ...
    public static class MyBloomFilter {
        ...
        //定义位图的大小，位图的数据结构不在此定义，使用redis来处理
        // 一般定义为2的幂次方
        private Integer cap;
        ...
        // 定义一个hash函数
        public Long hashCode(String value, Integer seed) { // 种子seed，减少冲突
            Long result = 0L;
            for (int i = 0; i < value.length(); i++) {
                result = result * seed + value.charAt(i); // 将字符串每位的ASCII码值相加，减少hash冲突，加上seed
            }

            // 防止溢出，由于cap为2的n次方，所以有如下位运算，加快速度，而不使用取余
            return result & (cap - 1);
        }
    ...
    public static class UvCountResultWithBloomFilter extends ProcessAllWindowFunction<UserBehavior, PageViewCount, TimeWindow> {
        ...
        public void open(Configuration parameters) throws Exception {
            ...
            myBloomFilter = new MyBloomFilter(1 << 29); // 一亿条数据，使用64MB的位图
        }
